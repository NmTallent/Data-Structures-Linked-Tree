...
public class PyramidScheme extends LinkedTree<Person> {
     ...
     * nodes that are above the node containing the target. This overload
     * takes a Person.
    ...
    /**
     * Similar to findNode, but rather than returning the node that
     * contains the target, instead returns a list of nodes above the
     * one that contains the target, in low to high level order.
    ...
    private ArrayList<MultiTreeNode<Person>> 
    	...
    	ArrayList<MultiTreeNode<Person>> ascendants = new ArrayList<MultiTreeNode<Person>>(); // create array for target ascendants
     ...
     * ensures the appropriate payment between the recruit and the recruiter,
     * as well as those above them in the hierarchy.
    ...
    /**
     * Performs all the transactions inherent to the system wherein a 
     * percentage of anything earned by an individual is passed upwards in the
     * hierarchy, such that each individual directly above gets an increasingly
     * smaller fraction of the money.
     ...
     * individuals in the pyramid scheme after it is discovered and halted.
     * Also severs each node from its children, such that no hierarchy exists
     * at all when the collapse is complete.
